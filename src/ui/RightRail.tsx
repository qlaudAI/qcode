    ...
    <>
      ...
      >
        ...
        {view === 'diff' && <DiffView workspacePath={workspacePath} />}
          ...
          {view === 'media' && <MediaView workspacePath={workspacePath} />}
        </div>
      </aside>
    </>
  );
...
type MediaItem = {
  /** Absolute path on disk — what we hand to openExternal. */
  absPath: string;
  /** Workspace-relative path — what we render. */
  relPath: string;
  /** File name only (last segment of relPath). */
  name: string;
  kind: MediaKind;
  /** Size in bytes. Null when stat failed. */
  size: number | null;
...
function MediaView({ workspacePath }: { workspacePath?: string | null }) {
  ...
  useEffect(() => {
    ...
    setItems(null);
    setError(null);
    void scanWorkspaceMedia(workspacePath).then(
      (res) => {
        if (cancelled) return;
        setItems(res);
      },
  ...
  if (!workspacePath) {
    return (
      <div className="flex flex-col items-center gap-2 px-4 py-8 text-center">
        ...
        <p className="text-[11.5px] leading-relaxed text-muted-foreground">
          Open a folder to see images, audio, and video tracked in
          your workspace.
        </p>
      </div>
    );
