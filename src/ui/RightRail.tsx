  ...
  return (
    <div>
      ...
      <ul className="space-y-0.5">
        {items.map((it) => (
          <MediaRow key={it.absPath} item={it} />
        ))}
      </ul>
    </div>
  );
...
function MediaRow({ item }: { item: MediaItem }) {
  ...
  //   • local-only: openExternal(absPath) → OS default app
  //   • cloud-only: openExternal(cloudUrl) → opens the qlaud download
  //     URL in the user's browser (a signed/authed download in v1
  //     will use a presigned URL; today the download endpoint reads
  //     the api key on the request, so direct browser-open works
  //     for the user who's already signed in)
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center justify-between gap-2">
            ...
            <div className="flex shrink-0 items-center gap-1.5">
              {/* Origin chip — only when cloud is involved. Local-only
               *  rows stay clean (the default visual). Cloud-only and
               *  both-origins flag the cloud presence so the user
               *  knows what's safe to access from another device. */}
              {item.origin !== 'local' && (
                <span
                  className="text-[10px] text-sky-600 dark:text-sky-400"
                  title={
                    item.origin === 'both'
                      ? 'Synced to qlaud cloud'
                      : 'Available only via qlaud cloud (not on this device)'
                  }
                  aria-label={
                    item.origin === 'both' ? 'Synced to cloud' : 'Cloud only'
                  }
                >
                  ☁
                </span>
